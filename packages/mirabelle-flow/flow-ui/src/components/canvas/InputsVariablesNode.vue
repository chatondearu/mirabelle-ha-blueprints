...
function isInputItem(item: FlowListItem): boolean {
  return item.group === 'input'
}
    ...
    <div class="mt-2 space-y-1 border-t border-neutral-700/70 pt-2">
      ...
      >
        <div class="flex items-center gap-1">
          ...
          <span v-if="item.group === 'input_alias'" class="text-[10px] text-teal-300/80">alias</span>
        </div>
        <template v-if="isInputItem(item)">
          <select
        ...
        <div v-else class="mt-0.5 truncate text-[10px] text-neutral-400">
          {{ itemDisplayValue(item) || '—' }}
        </div>
        <Handle
          type="source"
          :position="Position.Right"
          :id="isInputItem(item) ? `inp-${item.key}` : `var-${item.key}`"
